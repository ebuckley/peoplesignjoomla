...
define('PEOPLESIGN_PLUGIN_PHP_VERSION', 'psPlugPHP_1.5.1');
...
define( 'ERROR_EMPTY_KEY', 'received a private key that was all whitespace or empty');
define( 'NO_FRAMES_MESSAGE', 'Since it appears your browser does not support '
         . '\"iframes\", you need to click <a href=\"'.PEOPLESIGN_CHALLENGE_URL
         . '\">here</a> to verify you\'re a human.</p>');
define( 'ERROR_NO_SOCKET', 'Can\'t get socket to peoplesign host');
define( 'ERROR_EXCESSIVE_DATA', 'excessive data returned from peoplesign webservice call');
...
function getPeoplesignHTML ($peoplesignKey, $peoplesignOptions,
    ...
    if (isset($_SESSION[$clientLocation ."_peoplesignSessionID"])) {
    	$peoplesignSessionID = $_SESSION[$clientLocation ."_peoplesignSessionID"];	
    }
...
function isPeoplesignResponseCorrect ($peoplesignSessionID,
  ...
  if (!$peoplesignSessionID){
     $peoplesignSessionID = $_POST[PEOPLESIGN_CHALLENGE_SESSION_ID_NAME];
  }

  if (!$peoplesignResponseString) {
     $peoplesignResponseString = $_POST[PEOPLESIGN_CHALLENGE_RESPONSE_NAME];
  }
...
function getPeoplesignHTMLIFrame ($peoplesignSessionID,
    ...
    $peoplesignHTML = "<iframe src=\"" .PEOPLESIGN_CHALLENGE_URL
                  ."?".PEOPLESIGN_CHALLENGE_SESSION_ID_NAME
                        ."=$peoplesignSessionID\" "
